...
export function MemoryPanel({ userId, sessionId, disabled }: Props) {
  ...
  }, [category, load, memoryKey, memoryValue, sessionId, userId]);
  ...
  return <section className="card memory-panel">
    <div className="actions"><div><p className="eyebrow">Session Memory</p><h2>可审计会话记忆</h2></div>
      <button className="secondary" disabled={disabled} onClick={() => void load()}>刷新</button></div>
    <label className="inline-check"><input type="checkbox" checked={includeDeleted}
    ...
    <form className="memory-create" onSubmit={(event) => void create(event)}>
      <select value={category} disabled={disabled} onChange={(event) => {
      ...
      <select value={memoryKey} disabled={disabled} onChange={(event) => setMemoryKey(event.target.value)}>
        {KEYS[category].map((key) => <option key={key} value={key}>{key}</option>)}
      </select>
      <input value={memoryValue} disabled={disabled} placeholder="受控格式的值" onChange={(event) => setMemoryValue(event.target.value)} />
      <button disabled={disabled || !memoryValue.trim()} type="submit">创建</button>
    </form>
    ...
    <div className="memory-list">
      {entries.map((entry) => <article className="memory-entry" key={entry.entryId}>
        ...
        <div className="actions">
          <button className="secondary" disabled={disabled} onClick={() => void showEvidence(entry)}>证据</button>
          {!entry.deleted ? <><button className="secondary" disabled={disabled} onClick={() => void edit(entry)}>编辑</button>
            <button className="secondary" disabled={disabled} onClick={() => void remove(entry)}>删除</button></> : null}
        </div>
      </article>)}
    </div>
