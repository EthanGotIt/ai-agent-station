...
function statusLabel(status: string) {
  return ({ QUEUED: "排队中", ACTIVE: "执行中", WAITING_USER_INPUT: "等待确认", WAITING_EXTERNAL_ACTION: "等待外部动作", COMPLETED: "已完成", CANCELLED: "已取消", TIMED_OUT: "已超时", FAILED: "失败" } as Record<string, string>)[status] ?? status;
}
...
function QuestionCard({ value, disabled, onSubmit }: { value: QuestionCardState; disabled: boolean; onSubmit: (answers: Record<string, string>) => void }) {
  ...
  return <form className="decision-card v3-question-card" onSubmit={(event) => { event.preventDefault(); onSubmit({ decision }); }}>
    <div className="decision-heading"><CheckCircle2 aria-hidden="true" /><div><h2>{value.title}</h2><p>持久化检查点 · 版本 {value.version}</p></div></div>
    <p>{value.prompt}</p>
    <label>决定<select value={decision} disabled={disabled} onChange={(event) => setDecision(event.target.value)}><option value="CONFIRM">确认执行</option><option value="REJECT">拒绝执行</option></select></label>
...
function Turn({ turn }: { turn: ReturnType<typeof useThreadWorkspace>["turns"][number] }) {
  return <article className={`conversation-turn v3-turn turn-${turn.status.toLowerCase()}`}>
