...
export function ThreadWorkspace({ workspace, userId }: Props) {
  ...
  return <section className="thread-workspace" aria-labelledby="thread-workspace-heading">
    ...
    <div className="thread-layout">
      ...
      <aside className="execution-inspector trace-inspector"><div className="inspector-heading"><div><span className="eyebrow">EXECUTION LEDGER</span><h2>运行轨迹</h2><p>Item 是事实，SSE 是实时投影。</p></div><Archive aria-hidden="true" /></div>{workspace.error ? <p className="turn-error" role="alert"><CircleAlert aria-hidden="true" />{workspace.error}</p> : null}<ol className="trace-list">{workspace.trace.length === 0 ? <li className="trace-empty"><TerminalSquare aria-hidden="true" /><p>提交 Turn 后，这里会出现队列、工具、Workflow 和终态事件。</p></li> : workspace.trace.slice(-32).map((event) => <li className="trace-event" key={`${event.eventId}-${event.timestamp}`}><span className="trace-icon"><CheckCircle2 aria-hidden="true" /></span><div><div className="trace-meta"><strong>{event.type}</strong><time>{time(event.timestamp)}</time></div><p>{eventPayloadText(event.payload)}</p></div></li>)}</ol></aside>
    </div>
  </section>;
}
