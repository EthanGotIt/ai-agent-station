...
public final class ToolGuardPolicy {
    ...
    public static boolean isReadOnlyEvidenceTool(String toolName) {
        String normalizedName = normalize(toolName);
        return StringUtils.isNotBlank(normalizedName)
                && ToolRiskLevelEnumVO.LOW == assessRisk(normalizedName)
                && containsAny(normalizedName, LOW_RISK_HINTS)
                && !containsAny(normalizedName, HIGH_RISK_HINTS)
                && !containsAny(normalizedName, DANGEROUS_HINTS);
    }

    public static boolean isWriteOrDangerousTool(String toolName) {
        ToolRiskLevelEnumVO riskLevel = assessRisk(toolName);
        return ToolRiskLevelEnumVO.HIGH == riskLevel || ToolRiskLevelEnumVO.DANGEROUS == riskLevel;
    }
