...
public final class AfterSalesToolContractValidator {
    ...
    private static final Pattern ORDER_ID_VALUE_PATTERN = Pattern.compile("\"orderId\"\\s*:\\s*\"([^\"]+)\"");
    ...
    public ValidationResult validate(AfterSalesToolRequest request, String orderIdHint) {
        ...
        if (argumentsJson == null || !argumentsJson.trim().startsWith("{") || !argumentsJson.trim().endsWith("}")) {
            return ValidationResult.invalid("TOOL_ARGUMENT_INVALID", "工具参数不是合法 JSON");
        }
        Matcher keyMatcher = KEY_PATTERN.matcher(argumentsJson);
        while (keyMatcher.find()) {
            if (!ALLOWED_FIELDS.contains(keyMatcher.group(1))) {
                return ValidationResult.invalid("TOOL_ARGUMENT_INVALID", "工具参数只允许 orderId");
            }
        }
        Matcher valueMatcher = ORDER_ID_VALUE_PATTERN.matcher(argumentsJson);
        if (!valueMatcher.find()) {
            return ValidationResult.invalid("TOOL_ARGUMENT_INVALID", "orderId 格式非法");
        }
        String orderId = valueMatcher.group(1);
