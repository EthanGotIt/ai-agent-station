...
- 第二阶段验证：前端 typecheck、Vitest 28 项、生产构建和 Impeccable detector 均通过（detector 返回 `[]`）；服务保持关闭，MySQL 继续运行。下一步唯一动作是拆分后端 Runtime、Workflow 和 Spring AI Tool 职责，同时保持现有 API、事务和 FIFO 契约。
