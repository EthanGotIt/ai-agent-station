...
- `python -m scripts.convention_check`：当前失败；既有 checker 报告禁止文本和 Infrastructure Jackson 直接依赖规则，详见追踪矩阵，未通过修改规则掩盖。
- `python -m unittest discover -s scripts/tests -p "test_*.py"`：3 个测试中 2 个通过、1 个因上述 convention 问题失败。
