...
| API、SQL、配置、文档一致性 | 部分实现 | API 路径、Item envelope、DeepSeek 配置和 `.env.example` 已在后端提交中对齐；真实浏览器已核对 items/events 请求、`afterSequence` 游标和 SSE `item.*` 事件；`e83b9c9` 使规则检查与 Jackson 3 依赖边界一致；`mvn dependency:analyze -DskipTests` 构建成功但报告若干传递依赖未显式声明，旧数据库因未执行基线会缺少新列，运行手册已明确只能在可丢弃库执行基线 | P1 | 按实际调用路径审计 dependency analyze warning，核对 SQL 基线与配置文档并完成全契约矩阵 |
