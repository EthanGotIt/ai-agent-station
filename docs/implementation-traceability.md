...
| --- | --- | --- | --- |
| 1. 数据库与状态基线 | 已验证完成 | `8093d7a`、`6d40351`；V1/V2 恢复与订单字段迁移、V3 步骤/状态快照和 `(RUN_ID, STEP_NO)` 唯一约束已完成。当前库与专用校准库均由 Flyway 从版本 2 增量到 4，V4 `IDX_EXTERNAL_ACTION_THREAD_STATUS` 已存在；V3/V4 备份分别保留在 `C:\Users\23260\AppData\Local\Temp\commerce-guardian-agent-before-v3-commerce_guardian_agent-20260823.sql`、`C:\Users\23260\AppData\Local\Temp\commerce-guardian-agent-before-v3-commerce_guardian_agent_calibration_20260821-20260823.sql`、`C:\Users\23260\AppData\Local\Temp\commerce-guardian-agent-before-v4-commerce_guardian_agent-20260823.sql`、`C:\Users\23260\AppData\Local\Temp\commerce-guardian-agent-before-v4-commerce_guardian_agent_calibration_20260821-20260823.sql`；9 条订单、6 条物流事件保持不变 | 最终完整矩阵仍待执行 |
| 2. QuestionCard 与实时交互收口 | 已验证完成 | `394b5b2`、`fab72a3`、`49311ca`；动态字段、最多三选项、其他输入、Enter/Shift+Enter/IME、Escape、受限 Markdown、业务进度聚合、`stepNo` 快照和 SSE 断线恢复均有测试；开放 Question 快照接口无开放问题返回 204 | 订单 Workflow 的真实浏览器回归尚未执行 |
| 3. 订单发现、物流诊断与 V4 Pro 契约 | 已验证（浏览器待验） | `13500ba`、`56b631e`；`OrderSearchCriteria`、本地/HTTP `/orders/search`、结构化订单/物流 Item、V2 展示字段和 `deepseek-v4-pro` thinking 配置已实现；真实 V4 Pro 已完成订单搜索、物流时间线、SSE delta/取消、超时和敏感字段检查；`fcec19d` 将结果呈现为订单卡片、物流时间线和上下文动作入口 | 真实浏览器订单卡片/物流时间线的刷新、断线和 Thread 切换回归尚未验证 |
| 4. 统一 `ORDER_SERVICE` Workflow | 已验证（浏览器待验） | `49311ca`、`6d40351`；确定性候选/核验/多 Question/最终授权/owner Turn/退款、催发货、隐藏/恢复幂等命令、本地事务更新和 HTTP 远程适配已实现；Maven Core 56、Infrastructure 56、App 17 通过；校准库真实退款、隐藏、恢复均完成，重复回答/提交收到 409，命令与结果保持单行 | 真实浏览器端到端、催发货真实数据库动作和最终完整矩阵待执行 |
| 5. 能力集与产品化收尾 | 已实现（验收待闭合） | `6d40351`、`fcec19d`；Thread ACTIVE/ARCHIVED 列表、归档保护、回收站恢复、行内重命名、订单卡片动作、双栏消费者布局和移动端抽屉已实现；前端 typecheck、23 项 Vitest、production build 通过 | 真实浏览器全流程和最终 Python/Maven/npm 矩阵 |
...
- Maven 阶段 5 后端验证：全量 `mvn clean -DskipTests=false test` 通过 Core 56、Infrastructure 56、App 17；新增订单动作、Thread 生命周期和 HTTP 动作契约测试均通过。
- 前端阶段 5 验证：typecheck 通过；Vitest 23 个测试通过；production build 通过；组件替身脚本仍明确命名为 `test:component`，未冒充真实浏览器。
...
| 类型化 Item 与统一序列日志 | 已验证（浏览器待验） | Core `AgentItemTypeEnum`、`AgentItemModel` 和 `AgentItemPayloadModel` 强制 `schemaVersion=1 + kind + data` envelope；`ORDER_LIST`、`ORDER_DETAIL`、`LOGISTICS_TIMELINE` 和受控业务进度 Item 已投影，前端按 kind 聚合为订单卡片和物流时间线，不展示 Tool JSON、事件名或 Thinking | P0 | 订单结构化 Item 的真实浏览器断线/重载回归待验证 |
...
| Spring AI / DeepSeek 请求契约 | 已验证（浏览器待验） | `spring-ai-starter-model-deepseek` 保留 `stream().content()`、取消和超时分类；固定 `deepseek-v4-pro`，开启 thinking 与 `reasoning-effort=max`，`.env`/`.env.example` 已同步；真实 V4 Pro 已完成订单/物流 Tool Calling、SSE delta、取消、超时和敏感字段检查，未将 Thinking 或 delta 写入 Item、日志和前端 | P1 | 订单售后 Workflow 的真实浏览器流式 QuestionCard/外部动作恢复待验证 |
...
| 前端线程切换与 QuestionCard | 已实现（浏览器待验） | `useThreadWorkspace` 保留 generation、历史 AbortController、旧事件 Thread 过滤和切换期间禁用；QuestionCard 提交 `APPROVE/REJECT`，组件测试覆盖迟到历史、答案体、订单动作和 Thread 回收站/重命名；`91f2afb` 按结构化外部动作状态恢复 Turn 展示并接入人工重试 | P0 | 真实浏览器仍需验证多 Question、刷新/断线、归档保护和移动端抽屉 |
| API、SQL、配置、文档一致性 | 已实现（文档/矩阵待最终验收） | API/Item envelope/身份边界保持不变；V3/V4 增量 migration 增加 Run 步骤/状态、Question 步骤、外部动作索引；本地退款/催发货/隐藏/恢复与 HTTP `/orders/{id}/refund|expedite|visibility` 契约已同步；`GET /threads?status=ACTIVE|ARCHIVED`、前端列表和订单卡片契约已同步；`.env` 与 `.env.example` 的 `DEEPSEEK_MODEL` 均为 `deepseek-v4-pro` | P0 | 最终完整矩阵和文档 completed 结论尚未写入 |
