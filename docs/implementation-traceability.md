...
- 以上本地测试已补充专用 MySQL 启动、重启恢复和并发回答 HTTP 实证；真实浏览器已补充 Thread/QuestionCard/重载恢复证据，真实 DeepSeek 仍尚未验证，不能据此宣称目标完成。
...
| --- | --- | --- | --- | --- |
| 外部动作命令的 Lease、版本、重试与幂等 | 部分实现 | `ExternalActionCommandModel`、`MybatisExternalActionCommandStore` 已有版本/CAS、Lease、总尝试和重试周期；`ExternalActionOutcomeManager` 在命令 CAS 成功后于同一本地事务投影 WorkflowRun、Turn 和结构化 Item；专用 MySQL 已验证单 Worker 成功、冲突终态投影回滚后 Lease 接管并复用同一结果，以及双 Worker 竞争只产生一次执行；干净的重试耗尽/人工重试矩阵仍需补齐 | P0 | 在专用 MySQL 补充重试耗尽和人工重试幂等 |
...
| 外部动作成功/失败/人工重试 | 部分实现 | `ExternalActionOutcomeManager` 统一写入 `EXTERNAL_ACTION_STATUS`、`TURN_STATE`，命令/Workflow/Turn/Item 在本地事务内收敛；专用 MySQL 已验证远程夹具成功、投影冲突回滚、Lease 接管、结果表单行幂等和双 Worker CAS；干净的重试耗尽与人工重试不产生第二次副作用仍需补齐 | P0 | 补专用 MySQL 重试耗尽和人工重试验证 |
| 外部动作人工重试状态收口 | 部分实现 | `04a4c1c` 已验证 `MANUAL_RETRY_REQUIRED → WAITING_EXTERNAL_ACTION/COMPLETED`，人工重试成功不重写已失败 Turn；真实 API 已返回原 command/idempotencyKey，且结果映射缺陷已由 `9dba42b` 修复；干净的重试耗尽与人工重试重启恢复仍需验证 | P0 | 用专用 MySQL 完成干净人工重试和重启恢复 |
...
| 前端线程切换与 QuestionCard | 部分实现 | `useThreadWorkspace` 已有 generation、历史 AbortController、旧事件 Thread 过滤和切换期间禁用；QuestionCard 已提交 `APPROVE/REJECT`，组件测试覆盖迟到历史和答案体；真实浏览器已验证创建、重命名、切换、QuestionCard 拒绝、失败 Turn 和重载恢复，外部动作状态映射仍需审计 | P0 | 修复/验证外部动作状态映射及真实重连边界 |
...
本轮已完成八个代码里程碑：外部动作本地投影事务收口（`3e5e4a0`）、SSE 单连接回放/实时合并收口（`c927ca0`）、Turn 版本 CAS 与终态保护（`0c836c1`）、QuestionCard/WorkflowRun 状态机收口（`04a4c1c`）、Runtime/DeepSeek/Boot 兼容与超时分类收口（`c5ca160`）、Workflow 回答失败重试投影与 Item ID 边界收口（`dd7a5c3`）、SSE 异步连接结束边界收口（`821733c`）、外部动作结果实体映射收口（`9dba42b`）。当前全量 Maven 100 项通过，并完成专用 MySQL 启动、重启恢复和并发回答 HTTP 实证、ExternalAction Lease/CAS/回滚/幂等/双 Worker 实证，以及真实浏览器 QuestionCard/重载恢复实证；仍须完成真实浏览器人为断线、干净 ExternalAction 重试耗尽/人工重试、真实 DeepSeek 以及剩余 P0/P1 矩阵。

## 外部验证边界

已确认专用校准边界为本机 `127.0.0.1:3306/COMMERCE_GUARDIAN_AGENT_CALIBRATION_20260821`，当前只在该库导入基线；原 `COMMERCE_GUARDIAN_AGENT` 未重建。数据库日志和命令输出均未打印密码。真实浏览器已执行 Thread/QuestionCard/重载恢复流程；人为网络断开和真实 DeepSeek 尚未验证。当前环境没有可用的 DeepSeek 凭据，探针仅使用假值/本机不可达端点，因此真实模型验证必须继续标记为未验证，不能用替身测试替代。
