...
- Maven clean test：99 个测试通过（core 42、infrastructure 44、app 13）。
...
本轮已完成七个代码里程碑：外部动作本地投影事务收口（`3e5e4a0`）、SSE 单连接回放/实时合并收口（`c927ca0`）、Turn 版本 CAS 与终态保护（`0c836c1`）、QuestionCard/WorkflowRun 状态机收口（`04a4c1c`）、Runtime/DeepSeek/Boot 兼容与超时分类收口（`c5ca160`）、Workflow 回答失败重试投影与 Item ID 边界收口（`dd7a5c3`）、SSE 异步连接结束边界收口（`821733c`）。当前全量 Maven 99 项通过，并完成专用 MySQL 启动、重启恢复和并发回答 HTTP 实证，以及真实浏览器 QuestionCard/重载恢复实证；仍须完成真实浏览器人为断线、真实 DeepSeek 以及外部动作真实锁/事务矩阵。
