...
| 3. 订单发现、物流诊断与 V4 Pro 契约 | 部分实现 | `13500ba`；`OrderSearchCriteria`、本地/HTTP `/orders/search`、结构化订单/物流 Item、V2 展示字段和 `deepseek-v4-pro` thinking 配置已实现；校准库与当前库真实启动、V2 migration、Thread API 已通过；Convention Check、脚本 4 项、Maven 122 项、前端 22 项和 build/typecheck 均通过 | 真实 DeepSeek V4 Pro 多轮 Tool Calling、流式/取消/超时和真实浏览器订单卡片尚未验证；这些是外部证据缺口，不得标记为最终完成 |
...
| Spring AI / DeepSeek 请求契约 | 部分实现 | `spring-ai-starter-model-deepseek` 保留 `stream().content()`、取消和超时分类；本阶段固定 `deepseek-v4-pro`，开启 thinking 与 `reasoning-effort=max`，`.env`/`.env.example` 已同步；Spring AI 官方契约确认支持该模型和 thinking 配置 | P1 | 历史真实 DeepSeek 证据使用旧模型，必须重新执行 V4 Pro Tool Calling、流式、取消、超时和敏感信息检查 |
