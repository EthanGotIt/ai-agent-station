    ...
    {orders.length > 0 ? <div className="order-card-grid">{orders.map((order) => {
      ...
      return <article className="order-card" key={order.orderId}>
        ...
        {!deleted ? <div className="order-card-actions" aria-label={`${order.orderId} 可用操作`}>
          ...
          <button type="button" className="secondary danger-action" disabled={disabled} aria-busy={disabled} onClick={() => onAction(turn.turnId, order.orderId, "DELETE_ORDER")}><Trash2 aria-hidden="true" />删除记录</button>
        </div> : null}
      </article>;
    })}</div> : null}
...
function Turn({ turn, busy, pendingAction, retryingRunId, onRetry, onRetryTurn, onInspect, onAction }: { turn: ThreadViewTurn; busy: boolean; pendingAction: PendingOrderAction | null; retryingRunId: string | null; onRetry: (runId: string) => void; onRetryTurn: (turnId: string) => void; onInspect: (turnId: string) => void; onAction: (sourceTurnId: string, orderId: string, actionType: OrderActionType) => void }) {
...
export function ThreadWorkspace({ workspace, userId }: Props) {
  ...
  const create = async () => { setMobileSidebarOpen(false); await workspace.createThread(); };
