...
- 旧 Harness 主执行链路已删除，不再作为新增能力入口；不要新增或恢复 Action Loop 兼容路径。
- Flow Plan 已退出主执行链路，不要重新把 `Run -> Plan -> Step` 作为主执行模型。
- 当前迁移方向是 Spring AI 2 优先：`ChatClient + Advisor Chain + Tool Calling + MCP + Session`。

## Spring AI 2 兼容性结论

- `ChatClient`、`ToolCallingAdvisor` 与 `ToolCallbackResolver` 的基础能力可用，并由 `SpringAi2CompatibilityTest` 覆盖。
...
- 保留旧 Harness 中真正有价值的业务治理能力：证据策略、引用校验、证据不足拒答、运行态复盘和 SSE 可观测性。
