 ...
 */
@Repository
public class RagParentChildIngestionRepository implements IRagParentChildIngestionRepository {
    ...
    public void replaceDocument(RagIngestionDocumentVO document, List<RagIngestionChunkVO> chunks) {
        ...
        mysqlJdbcTemplate.batchUpdate("""
                        INSERT INTO ai_rag_chunk (
                            rag_id, doc_id, chunk_id, parent_chunk_id, chunk_level, chunk_type, chunk_text, metadata_json, status, create_time, update_time
                        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
                        """,
                chunks,
                chunks.size(),
