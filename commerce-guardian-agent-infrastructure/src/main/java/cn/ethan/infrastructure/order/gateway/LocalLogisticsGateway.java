...
import cn.ethan.infrastructure.order.mapper.DemoLogisticsEventMapper;
...
public final class LocalLogisticsGateway implements LogisticsGateway {
    ...
    public List<LogisticsEventModel> findTrace(String orderId, String userId) {
        if (orders.findOrder(orderId, userId).status() != cn.ethan.core.order.enums.OrderLookupStatusEnum.FOUND) {
            return List.of();
        }
